    ...
    if request.method == "POST":
        ...
        if selected_address_id:
            ...
            address = Address.objects.create(
                customer=customer, name=name, email=email, line_1=address1
            )
        ...
        for item in cart_items:
            try:
                ...
                category_offers = CategoryOffer.objects.filter(category_id=category_id)[
                    :1
                ]
                ...
                if payment_option == "cod":
                    ...
                    try:
                        ...
                        if "id" in payment:
                            ...
                            messages.error(
                                request,
                                "Error creating Razorpay order: {}".format(
                                    payment.get("error")
                                ),
                            )
                        ...
                        messages.error(
                            request,
                            "Error during Razorpay integration: {}".format(str(e)),
                        )
            else:
                messages.error(
                    request,
                    "The selected payment option is not available now. Please choose another option.",
                )
        else:
            for field in form:
...
def update_cart(request, product_id):
    if not request.user.is_authenticated:
        ...
        cart_item = get_object_or_404(
            CartItem, product_id=product_id, customer=request.user
        )
...
def add_to_wish_list(request, product_id):
    ...
    if isinstance(request.user, AnonymousUser):
        ...
        if wishlist_item_exists:
            return JsonResponse(
                {"message": "Product is already in your wishlist."}, status=400
            )
